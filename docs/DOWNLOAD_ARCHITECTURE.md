...
调用方可通过 `UpdateDownloadControl.SetUseAccelerationNodes(false)` 在下载中要求切换到 `github-direct` 官方直连；重新开启时恢复加速节点策略。`RequestNextAcceleratedNode()` 用于用户显式跳过当前加速节点并切换到下一加速节点。两种操作只中断当前节点尝试，不取消整体下载、不会跳过大小或 SHA-256 校验。
